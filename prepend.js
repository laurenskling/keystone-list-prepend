...
function add () {
    ...
    // var keys = Object.keys(obj); // reverse the nested object
  ...
  _.forEach(args.reverse(), function (def) {
  // _.forEach(args, function (def) { // reverse the args, so we unshift from the end
  	self.schemaFields.unshift(def);
    // self.schemaFields.push(def);
		if (typeof def === 'string') {
			if (def === '>>>') {
				self.uiElements.unshift({
